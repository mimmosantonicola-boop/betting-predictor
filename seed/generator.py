...
class SeedGenerator:
    ...
    def _team_profile(self, team_name: str, form: TeamForm, stats: TeamStats,
        ...
        form_details = []
        for i, (r, gs, gc) in enumerate(
            zip(form.results[-5:], form.goals_scored[-5:], form.goals_conceded[-5:])
        ):
    ...
    def _statistical_matchup(self, report: MatchReport) -> str:
        ...
        # Projected total corners for this match
        projected_corners = round(hs.corners_pg + as_.corners_pg, 1)
        projected_goals = round(hs.xg_pg + as_.xga_pg + as_.xg_pg + hs.xga_pg, 1) / 2
        projected_goals = round((hs.xg_pg + as_.xg_pg), 1)
        projected_cards = round(hs.yellow_cards_pg + as_.yellow_cards_pg, 1)
        ...
        return (
            ...
            f"| Corners/game | {hs.corners_pg} | {as_.corners_pg} |\n"
            f"| Yellow cards/game | {hs.yellow_cards_pg} | {as_.yellow_cards_pg} |\n"
            ...
            f"**Projected match total corners:** ~{projected_corners}  \n"
            f"**Projected match xG (combined):** ~{projected_goals}  \n"
            f"**Projected yellow cards:** ~{projected_cards}  \n"
        )
